...
def hide_text(image_path, secret_text, output_path):
    # Open gambar
    image = Image.open(image_path).convert("RGBA")  # Convert to RGBA to handle alpha channel
...
def extract_text(image_path):
    ...
    # Menampilkan text rahasia dari gambar
    pixels = image.load()
    binary_secret_text = ""
    for i in range(image.width):
        for j in range(image.height):
    ...
    # Mengubah teks binar menjadi ASCII
    secret_text = ""
...
image_path = 'bananaref.png'  # Jalur file gambar asli
secret_text = 'This is a secret message.'
output_path = 'bananaref.png'  # Jalur file gambar yang dihasilkan
hide_text(image_path, secret_text, output_path)
 
# Ekstraksi teks rahasia dari gambar
